...
def top_features(data, classifier = ''):
    ...
    importance_df = pd.DataFrame(importance, columns = data.drop(['Label'], axis=1).columns)
...
def prepare_ISCX_2014_data(PATH = '', INPUT_FILE_NAME = ''):
    ...
    print('Processing File: ' + str(INPUT_FILE_NAME) + ' DATA shape: ' + str(data.shape))
    ...
    if network_traffic:
        data= data.drop(['Src IP', 'Src Port', 'Dst IP', 'Dst Port', 'Timestamp', 'Protocol'], axis=1).copy()
        if DEBUG:
    ...
    # =============================================================================================
        ...
        if DEBUG:
            ...
            print("Black_hole_1_Instances:" + str(len(indx_Black_hole_1)) + " ---> "+ str(round(len(indx_Black_hole_1)/total_instances*100, 6)) + " %")
            ...
            print("bin_Zeus_Instances:" + str(len(indx_bin_Zeus)) + " ---> "+ str(round(len(indx_bin_Zeus)/total_instances*100, 6)) + " %")
            ...
            if CTU_NERIS: 
                print("CTU_neris_instances:" + str(len(indx_CTU_neris)) + " ---> "+ str(round(len(indx_CTU_neris)/total_instances*100, 6)) + " %")
    ...
    # This cell labels the 'Label' column in the data frame to 1 where the particular botnet was found
        ...
        if CAT_FEATURES == 1:
            ...
            if USE_ONE_BOT:
                ...
                data = pd.concat([benign_samples, one_bot_samples]).reset_index(drop=True) #Augmenting with real botnets
                ...
                data_frame = pd.concat([data_frame, bot_2_samples]).reset_index(drop=True) #Augmenting with real botnets
                ...
                data_frame = pd.concat([data_frame, bot_3_samples]).reset_index(drop=True) #Augmenting with real botnets
                ...
                data_frame = pd.concat([data_frame, bot_4_samples]).reset_index(drop=True) #Augmenting with real botnets
                ...
                data = pd.concat([benign_samples, bot_samples]).reset_index(drop=True) #Augmenting with real botnets
    ...
    data = data.replace([np.inf, -np.inf], np.nan).dropna(how="any").reset_index(drop=True).copy()
...
def prepare_cic_2017_data(PATH = '', INPUT_FILE_NAME = '', CSV_ONE_BOT = 0):
    ...
    if network_traffic: # This is just for testing purposes. I can test this code with another tabular dataset so this section will be skipped in that case by disabling the network_traffic flag in the start of this code.
    # =============================================================================================
    # Choose only Flow ID and Label columns for labeling purposes in another data frame
    # ============================================================================================= 
...
def prepare_UNSW_IoT(PATH = '', INPUT_FILE_NAME = ''):
    ...
    data= data.drop(['pkSeqID', 'saddr', 'sport', 'daddr', 'dport', 'seq', 'category' , 'subcategory', 'method'], axis=1).copy()
    if DEBUG:
        print('data_df after removing categorical features')
        print(data.shape)
...
def prepare_DARKNET_2020_data(PATH = '', INPUT_FILE_NAME = '', CSV_ONE_BOT = 0):
    ...
    data= data.drop(['Flow ID', 'Src IP', 'Src Port', 'Dst IP', 'Dst Port', 'Protocol', 'Timestamp', 'Label_1'], axis=1).copy()   
